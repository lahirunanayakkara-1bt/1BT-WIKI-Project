...
export function DraftManagerSidebar({ isOpen, toggleSidebar }: DraftManagerSidebarProps) {
  ...
  return (
    <div
      ref={sidebarRef}
      className="relative flex h-full shrink-0 flex-col border-r border-[#E5E7EB] bg-white z-20"
      style={{ width: 320 }}
    >
      {/* Content Wrapper that clips when collapsed */}
      <div className="overflow-hidden w-full h-full">
        <div ref={contentRef} className="flex h-full w-[320px] flex-col p-5">
          <button className="flex w-full items-center justify-center gap-2 rounded-lg bg-[#CC0000] px-4 py-3 text-sm font-bold text-white shadow-sm hover:bg-[#A80000] transition-colors">
        ...
        <div className="relative mt-4">
          ...
          <input
            ...
            className="w-full rounded-lg border border-[#E5E7EB] bg-[#F5F5F5] py-2 pl-9 pr-3 text-sm text-[#1A1A1A] placeholder-[#9CA3AF] focus:border-[#CC0000] focus:outline-none focus:ring-1 focus:ring-[#CC0000] transition-all"
          />
        </div>
          ...
          <div className="flex flex-col gap-3">
            {mockDrafts.map((draft, i) => (
              ...
              >
                ...
                <h4 className="font-bold text-[#1A1A1A] line-clamp-1 pr-6">{draft.title}</h4>
                ...
                <div className="mt-4 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-[#9CA3AF]">{draft.date}</span>
                  </div>
                  <span className="text-xs font-bold text-[#CC0000]">{draft.tag}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="mt-4 flex items-center justify-between border-t border-[#E5E7EB] pt-4">
          ...
          <span className="text-[10px] text-[#9CA3AF]">v1.2</span>
        </div>
      </div>
    </div>
      ...
      <button
        ...
        className="absolute top-6 z-30 flex h-10 w-10 items-center justify-center rounded-full border border-[#E5E7EB] bg-white text-[#6B7280] shadow-md hover:text-[#1A1A1A] hover:bg-[#F0F0F0] transition-all duration-300"
