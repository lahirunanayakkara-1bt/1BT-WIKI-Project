...
describe('MyArticlesPage', () => {
  ...
  it('shows a loading state while the user is loading', () => {
    ...
    expect(screen.queryByTestId('my-articles-list-stub')).not.toBeInTheDocument();
  });
  ...
  it('prompts sign-in when there is no user', () => {
    ...
    expect(screen.getByText('Please sign in to view your articles.')).toBeInTheDocument();
    expect(screen.queryByTestId('my-articles-list-stub')).not.toBeInTheDocument();
  });
  ...
  it('renders the article list for a signed-in user', () => {
    ...
    expect(screen.getByRole('heading', { name: 'My Articles' })).toBeInTheDocument();
